...
class TopDownCore(tf.keras.layers.Layer):
    def __init__(self, units, hidden_size, **kwargs):
        ...
        self.units = units
        # Use GRUCell instead of LSTMCell
        self.topdown = tf.keras.layers.GRUCell(units)
        self.language = tf.keras.layers.GRUCell(units)
    ...
    def call(self, xt, avg_features, features, state):
        ...
        prev_h = state[1]
        # Concatenate state[1] and avg_features
        topdown_input  = tf.concat([prev_h, avg_features, xt], -1)
        # TopDown GRUCell Layer
        h_topdown, _ = self.topdown(topdown_input, state[0])
        # Attention Layer
        ...
        # Language GRUCell Layer
        h_language, _  = self.language(language_input, state[1])
        # Update the state
        state = [h_topdown, h_language]
        return h_language, state

# Caption decoder of TopDown model
class TopDownDecoder(tf.keras.Model):
    def __init__(self, embedding_dim, units, vocab_size, hidden_size):
